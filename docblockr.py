...
class DocblockrCommand(sublime_plugin.TextCommand):
    ...
    def initialize(self, v, inline=False):
        ...
        self.settings = sublime.load_settings("Doc​Blockr.sublime-settings")
...
class DocblockrTrimAutoWhitespace(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        lineRegion = v.line(v.sel()[0])
        line = v.substr(lineRegion)
        spaces = max(0, sublime.load_settings("Doc​Blockr.sublime-settings").get("docblockr_indentation_spaces", 1))
...
class DocblockrWrapLines(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        v = self.view
        settings = sublime.load_settings("Doc​Blockr.sublime-settings")
